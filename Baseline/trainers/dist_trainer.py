...
class FER2013Trainer(Trainer):
    ...
    def __init__(self, model, train_set, val_set, test_set, configs):
        ...
        # for dist
        self._ngpus_per_node = torch.cuda.device_count()
        if self._distributed == 1:  # DISTINGUST WITH MULTIPROCESSING DISTRIBUTED
            self._world_size = self._ngpus_per_node * WORLD_SIZE
    ...
    def train(self, gpu, ngpus_per_node, args):
        ...
        if self._distributed:
            torch.distributed.init_process_group(
                backend="nccl", world_size=torch.cuda.device_count(), rank=1
            )
            self._model.cuda()
